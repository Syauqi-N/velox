...
export default function RecentCalls() {
  ...
  return (
    <section className="card overflow-hidden">
      ...
      {state === "loading" ? <p className="px-4 py-5 text-sm text-[var(--muted)]">Memuat call.</p> : state === "error" ? <div className="px-4 py-5 text-sm"><p className="text-[var(--down)]">Call tidak dapat dimuat.</p><button type="button" onClick={() => void load()} className="mt-2 font-medium underline">Coba lagi</button></div> : calls.length === 0 ? <p className="px-4 py-5 text-sm text-[var(--muted)]">Belum ada call.</p> : <ul className="divide-y divide-[var(--border)]">{calls.map((call) => <li key={call.id} className="px-4 py-4"><div className="flex items-center justify-between gap-2"><Link href={`/stock/${call.ticker}`} className="font-semibold hover:text-[var(--accent)]">{call.ticker.replace(".JK", "")}</Link><span className="rounded-md border border-[var(--border)] bg-[var(--card-hover)] px-2 py-0.5 text-xs">{call.action} · {call.status === "OPEN" ? "Aktif" : "Selesai"}</span></div><p className="mt-1 text-xs tabular-nums text-[var(--muted)]">Entry {formatPrice(call.entryPrice)} · Target {formatPrice(call.targetPrice)}</p>{call.reason && <p className="mt-1 text-xs leading-5 text-[var(--muted)]">{call.reason}</p>}<CallChart symbol={call.ticker} height={150} />{isAdmin && call.status === "OPEN" && <button type="button" disabled={busy} onClick={() => void closeCall(call)} className="mt-3 text-xs font-medium underline">Tutup Call</button>}</li>)}</ul>}
    </section>
  );
}
